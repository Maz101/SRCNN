...
conv1 = tf.nn.bias_add(tf.nn.conv2d(inputs,weights['w1'],strides=[1,1,1,1],padding='SAME'),biases['b1'])
print(conv1.shape)
...
# variabiles (w1, w2, w3)
weight1 = model['w1']
fig, axs = plt.subplots(8,8, figsize=(11,11))
axs = axs.ravel()
print(weight1.shape)
for f in range(weight1.shape[3]):
    ex_filter = weight1[:,:,:,f]
    ex_filter = np.squeeze(ex_filter)
    axs[f].imshow(ex_filter)

weight2 = model['w2']
fig3, axs = plt.subplots(4,8, figsize=(3,3))
axs = axs.ravel()
for f in range(weight2.shape[3]):
    ex_filter2 = weight2[:,:,:,f]
    ex_filter2 = np.squeeze(ex_filter2)
    axs[f].imshow(ex_filter)    
    
    
weight3 = model['w3']
fig2, ax3 = plt.subplots(4,8, figsize=(7,7))
ax3 = ax3.ravel()
for f in range(weight3.shape[2]):
    ex_filter3 = weight3[:,:,f,:]
    ex_filter3 = np.squeeze(ex_filter3)
    ax3[f].imshow(ex_filter3) 
...
groundinput_ = np.expand_dims(np.expand_dims(groudtruth_image,axis=0), axis=-1)

# run the session
# here you can also run to get feature map like 'conv1' and 'conv2'
output_ = sess.run(conv3, feed_dict={inputs: input_})

output2_ = sess.run(conv2, feed_dict={inputs: input_})
fig4, ax4 = plt.subplots(4,8, figsize=(255,255))
ax4 = ax4.ravel()
for f in range(output2_.shape[3]):
    conv_2 = output2_[:,:,:,f]
    conv_2 = np.squeeze(conv_2)
    print("conv_2 shape:",conv_2.shape)
    ax4[f].imshow(conv_2) 

    
# output3_ = sess.run(conv1, feed_dict={inputs: input_})
# fig5, ax5 = plt.subplots(8,8, figsize=(255,255))
# ax5 = ax5.ravel( )
# for f in range(output3_.shape[3]):
#     conv_1 = output3_[:,:,:,f]
#     conv_1 = np.squeeze(conv_1)
#     ax5[f].imshow(conv_1)
...
superresolution = np.reshape(output_,(255,255))
groundtruth = np.reshape(groundinput_,(255,255))
blurred = np.reshape(input_,(255,255))

scipy.misc.imsave('image/Blurry_photo.png',blurred)
scipy.misc.imsave('image/Super-resolution.png',superresolution)
scipy.misc.imsave('image/Ground_Truth.png',groundtruth)

plt.show()

PSNR1 = skimage.measure.compare_psnr(groundtruth.astype(np.float32),superresolution.astype(np.float32))
PSNR2 = skimage.measure.compare_psnr(groundtruth.astype(np.float32),blurred_image.astype(np.float32))

print("Ground truth vs superresolution: ",PSNR1)
print("Ground truth vs blurred image: ", PSNR2)
